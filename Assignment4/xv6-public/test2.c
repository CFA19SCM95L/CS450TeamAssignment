...
int main() {
  int ext_fd = open("extentfile2.txt", O_CREATE|O_RDWR|O_EXTENT);
	if(ext_fd < 0) {
	...
	int norm_fd = open("pointerfile2.txt", O_CREATE|O_RDWR);
	if(norm_fd < 0) {
	...
	printf(1, "Attempting to write 5KB into extent-based file ... \n\n");
	...
	printf(1, "Attempting to write 5KB into pointer-based file ... \n\n");
	...
	printf(1, "Write to pointer based file successful. \n\n");
